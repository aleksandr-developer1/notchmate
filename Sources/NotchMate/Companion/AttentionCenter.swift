    ...
    var items: [AttentionItem] {
        ...
        for s in env.agents.sessions.values where s.state == .waiting {
            out.append(AttentionItem(id: "wait-\(s.id)", icon: s.agent.fallbackSymbol, appIcon: s.agent.icon, tint: .orange,
                                     title: "\(s.agent.title) ждёт ответа", subtitle: "\(s.project) · \(Self.ago(s.updatedAt))",
                                     actionTitle: "Перейти", action: .openAgent(s.agent, host: s.hostBundleID), dismissable: false, priority: 100))
        }

        if let e = env.calendar.next, e.start.timeIntervalSinceNow < 15 * 60, e.end > now {
            let when = e.isNow ? "идёт сейчас" : "через \(max(e.minutesUntil, 1)) мин"
            out.append(AttentionItem(id: "meet-\(e.id)", icon: "calendar", appIcon: nil, tint: e.color,
                                     title: e.title, subtitle: when,
                                     actionTitle: e.meetingURL != nil ? "Подключиться" : nil,
        ...
        for r in env.reminders.items {
            ...
            let when = late ? (r.hasTime ? "просрочено · \(f.string(from: r.due))" : "просрочено")
                : (r.hasTime ? "в \(f.string(from: r.due))" : "сегодня")
            out.append(AttentionItem(id: "rem-\(r.id)", icon: "checklist", appIcon: nil, tint: late ? .red : r.color,
                                     title: r.title, subtitle: when,
                                     actionTitle: "Готово", action: .completeReminder(r.id), dismissable: true,
        ...
        if env.focus.phase == .finished {
            ...
            out.append(AttentionItem(id: "pomo-\(env.focus.completedInCycle)-\(isWork)", icon: isWork ? "cup.and.saucer.fill" : "play.fill", appIcon: nil,
                                     ...
                                     title: isWork ? "Помидор завершён" : "Перерыв закончился",
                                     subtitle: isWork ? "Время отдохнуть" : "Возвращаемся к работе",
                                     actionTitle: isWork ? "Перерыв" : "К работе", action: .startBreak, dismissable: true, priority: 80))
        }
        ...
        if let issue = env.jira.activeIssue, let original = issue.originalEstimate {
            ...
            if left < 3600 {
                out.append(AttentionItem(id: "est-\(issue.key)-\(left < 0)", icon: "exclamationmark.triangle.fill", appIcon: nil,
                                         ...
                                         title: left < 0 ? "\(issue.key): эстимейт превышен" : "\(issue.key): эстимейт почти исчерпан",
                                         subtitle: left < 0 ? "сверх оценки \(JiraService.format(-left))" : "осталось \(JiraService.format(left))",
                                         actionTitle: "Открыть", action: .openJira(issue.key), dismissable: true, priority: 70))
            }
        }
        ...
        for key in newIssues {
            ...
            out.append(AttentionItem(id: "new-\(key)", icon: "sparkles", appIcon: nil, tint: Theme.jira,
                                     title: issue?.summary ?? key, subtitle: issue?.summary == nil ? "назначена на вас" : key,
                                     actionTitle: "Открыть", action: .openJira(key), dismissable: true, priority: 60))
        }

        switch env.calls.stage {
        case .recording:
            out.append(AttentionItem(id: "rec", icon: "record.circle.fill", appIcon: nil, tint: .red,
                                     title: "Идёт запись созвона", subtitle: FocusTimer.format(env.calls.elapsed),
                                     actionTitle: "Остановить", action: .stopCall, dismissable: false, priority: 120))
            if !env.copilot.isActive {
                out.append(AttentionItem(id: "copilot", icon: "sparkles", appIcon: nil, tint: Color(red: 0.62, green: 0.55, blue: 1.0),
                                         title: "Помощник на встрече", subtitle: "подскажет, что спросить и что в коде · ⌃⌥H",
                                         actionTitle: "Включить", action: .meetingAssistant, dismissable: true, priority: 118))
            }
        case .transcribing, .summarizing:
            out.append(AttentionItem(id: "rec-work", icon: "waveform", appIcon: nil, tint: Theme.jira,
                                     title: env.calls.stage == .transcribing ? "Расшифровываю созвон" : "Готовлю протокол",
                                     subtitle: "на этом компьютере", actionTitle: nil, action: .none, dismissable: false, priority: 119))
        case .idle:
            if env.calls.micActive {
                out.append(AttentionItem(id: "mic", icon: "mic.fill", appIcon: nil, tint: .orange,
                                         title: "Идёт разговор?", subtitle: env.calls.micStatusText,
                                         actionTitle: "Записать", action: .startCall, dismissable: true, priority: 95))
            }
            if let first = CallRecorder.pendingRecordings().first {
                out.append(AttentionItem(id: "pending-\(first.lastPathComponent)", icon: "waveform.badge.exclamationmark", appIcon: nil, tint: .orange,
                                         title: "Запись без протокола", subtitle: first.lastPathComponent,
                                         actionTitle: "Обработать", action: .processCalls, dismissable: true, priority: 94))
            }
        }
        ...
        if let note = env.calls.lastNote, !note.isEmpty, !dismissed.contains("call-\(note)") {
            out.append(AttentionItem(id: "call-\(note)", icon: "text.document.fill", appIcon: nil, tint: Theme.obsidian,
                                     title: "Протокол созвона готов", subtitle: (note as NSString).lastPathComponent,
                                     actionTitle: "Открыть", action: .openNote(note), dismissable: true, priority: 118))
        }
        if let err = env.calls.lastError, !dismissed.contains("callerr-\(err)") {
            out.append(AttentionItem(id: "callerr-\(err)", icon: "exclamationmark.triangle.fill", appIcon: nil, tint: .orange,
                                     title: "Запись созвона", subtitle: err, actionTitle: nil, action: .none, dismissable: true, priority: 117))
        }

        for (id, f) in finished {
            out.append(AttentionItem(id: id, icon: "checkmark.circle.fill", appIcon: f.agent.icon, tint: .green,
                                     title: "\(f.agent.title) закончил", subtitle: "\(f.project) · \(FocusTimer.format(TimeInterval(f.duration))) · \(Self.ago(f.at))",
                                     actionTitle: "Перейти", action: .openAgent(f.agent, host: f.host), dismissable: true, priority: 50))
        }
        ...
        for (id, f) in ciFailures {
            ...
            out.append(AttentionItem(id: id, icon: "xmark.octagon.fill", appIcon: nil, tint: .red,
                                     title: "CI упал: \(f.ci.failedCheck ?? f.ci.title)",
                                     subtitle: f.ci.prNumber.map { "\(f.repo) · PR #\($0)" } ?? f.repo,
                                     actionTitle: "Открыть", action: .openGit, dismissable: true, priority: 80))
        }
